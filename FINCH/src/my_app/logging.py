...
from src.my_app.utils import load_config
class TensorBoardLogger:
    ...
    def log_metrics(self, metrics_dict, step, prefix=""):
            ...
            self.writer.flush()
    def log_number_of_trainable_parameters(self, model:nnx.Module):
        state = nnx.state(model)
        total_params = sum(x.size for x in jax.tree_util.tree_leaves(state.filter(nnx.Param)))
        print(total_params)
        with self.writer.as_default():
                    tf.summary.scalar("Config/TotalParams", total_params, step=0)

                    # Using nnx.tabulate for the model summary
                    dummy_image_size = load_config()['dataset']['image_size'][0]
                    num_channels = load_config()['dataset']['channels']
                    dummy_image = jnp.ones((1, dummy_image_size, dummy_image_size, num_channels))
                    #summary_str = nnx.tabulate(model, dummy_image)

                    #tf.summary.text("Config/ModelSummary", f"```\n{summary_str}\n```", step=0)
